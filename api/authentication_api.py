...
def get_auth_cookie(login: str = LOGIN, password: str = PASSWORD):
    ...
    with step("Получить токен в ответе метода /v1/validate-login-password"):
    ...
    with step("Получить куку в ответе метода /logon"):
        ...
        if not cian_cookie:
            raise RuntimeError("Кука _CIAN_GK не найдена в ответе метода logon")
